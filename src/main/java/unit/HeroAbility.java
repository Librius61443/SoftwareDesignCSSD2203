...
public enum HeroAbility {
  ...
  HEAL(35) {
    ...
    public void execute(Hero caster, List<Unit> allies, List<Unit> enemies, List<Unit> selectedTargets) {
      if (!caster.spendMana(manaCost())) return;
      List<Hero> targets;
      if (caster.isHybrid(HybridClass.PRIEST)) {
  ...
  CHAIN_LIGHTNING(40) {
    ...
    public void execute(Hero caster, List<Unit> allies, List<Unit> enemies, List<Unit> selectedTargets) {
      ...
      for (int i = 0; i < aliveEnemies.size(); i++) {
        ...
        int divisor = caster.isHybrid(HybridClass.INVOKER) ? 2 : 4;
  ...
  REPLENISH(80) {
    @Override
    public void execute(Hero caster, List<Unit> allies, List<Unit> enemies, List<Unit> selectedTargets) {
      int manaCost = caster.isHybrid(HybridClass.WIZARD) ? 40 : manaCost();
      if (!caster.spendMana(manaCost)) return;
  ...
  private static void maybeStunTarget(Hero caster, Unit target) {
    if (caster.isHybrid(HybridClass.KNIGHT) && target.isAlive()
