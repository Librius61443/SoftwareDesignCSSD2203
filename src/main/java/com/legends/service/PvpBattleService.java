...
public class PvpBattleService {
    ...
    public PvpBattleResult battleFirstSavedParties(String challenger, String opponent) {
        return battleWithSelectedParties(challenger, 0, opponent, 0);
    }

    public PvpBattleResult battleWithSelectedParties(String challenger, int challengerIdx,
                                                      String opponent, int opponentIdx) {
        Party challengerParty = partyService.getSavedParty(challenger, challengerIdx);
        Party opponentParty   = partyService.getSavedParty(opponent, opponentIdx);
